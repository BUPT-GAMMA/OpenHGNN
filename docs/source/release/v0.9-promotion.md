...
OpenHGNN 是基于 DGL 构建的异质图神经网络工具包，因此 v0.9 更强调 DGL-first 的实现原则。新增模型的图结构、采样流程和消息传递应基于 DGLGraph、DGLHeteroGraph、DGL 采样器、DGL 消息传递接口和 `dgl.nn` 组件，尽量复用 OpenHGNN 已有的 `model`、`trainerflow`、`dataset`、`task` 和 `config` 体系。

新增模型不应以纯 PyTorch 张量图计算替代 DGL 的图表示和消息传递，也不应绕过 OpenHGNN 的统一任务入口另起一套训练流程。对于确实需要特殊预处理或额外数据组件的模型，文档中需要明确说明依赖边界和使用原因，但模型接入仍应回到 OpenHGNN 的 DGL 图结构、统一任务入口和训练流程中。HGSketch 目前提供组件级验证，并在文档中说明图级任务支持仍需继续完善；RelGT 涉及 RelBench 数据准备，因此在模型文档中单独说明其可选数据处理依赖。
