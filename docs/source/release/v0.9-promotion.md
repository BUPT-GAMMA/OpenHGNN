...
OpenHGNN 始终围绕 DGL 构建异质图学习能力。v0.9 继续强化这一特点，让新增模型能够更好地复用 DGL 的异构图表示、采样流程、消息传递机制和图神经网络算子。

对用户来说，这意味着不同模型可以尽量使用一致的数据、任务和训练入口，减少在不同复现代码之间切换的成本。对贡献者来说，也可以把主要精力放在模型创新本身，而不是重复搭建图结构处理、训练循环和评估流程。

通过更统一的 DGL 模型体验，OpenHGNN 0.9 希望让前沿异质图模型更容易被发现、运行、比较和继续扩展。
