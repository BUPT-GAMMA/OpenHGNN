...
from .hcm_dataset import HCMDataset
...
@register_dataset("hcm_recommendation")
class HCM_recommendation(RecommendationDataset):
    r"""
    Which is used in HCMGNN recommendations.
    """
    def __init__(self, dataset_name, *args, **kwargs):
        super(RecommendationDataset, self).__init__(*args, **kwargs)
        args = kwargs.get('args', None)
        # dataset = HCMDataset(name=dataset_name, raw_dir='', args=args)
        self.hcm_dataset = HCMDataset(name=dataset_name, raw_dir='', args=args)

        # After HCMDataset.process() runs, it will have populated its attributes
        # with the DGL graph, features, and paths to the generated CSVs.
        # We can now access these from the hcm_dataset instance.
        self.g = self.hcm_dataset._g # The DGL graph
        self.features = self.hcm_dataset.features # Dictionary of feature tensors
        self.in_size = self.hcm_dataset.in_size # Dictionary of feature dimensions

        # Paths to the independent test set CSVs, generated by HCMDataset.process()
        # These paths are used by HCMGNN_trainer to load the data.
        self.train_data_pos_path = self.hcm_dataset.train_data_path # Path to independent train pos
        self.train_data_neg_path = self.hcm_dataset.train_neg_data_path # Path to independent train neg
        self.test_data_pos_path = self.hcm_dataset.test_data_pos_path # Path to independent test pos
        self.test_data_neg_path = self.hcm_dataset.test_data_neg_path # Path to independent test neg

    def get_split(self):
        """
        Returns the DGL graph, and the independent training and testing data arrays.
        This method is designed to be called by the HCMGNN_trainer to get all necessary data.
        It returns 3 values to match the unpacking in the Recommendation class.
        """
        # Return:
        # 1. The DGL heterogeneous graph (self.g)
        # 2. The combined independent training data (self.hcm_dataset.train_data_indep)
        # 3. The combined independent testing data (self.hcm_dataset.test_data_indep)
        return self.g, self.hcm_dataset.train_data_indep, self.hcm_dataset.test_data_indep

    def get_train_data(self):
        """
        Returns the combined independent training data (pos + neg) as a NumPy array.
        This is provided for convenience if a direct array is needed,
        though the trainer might prefer paths.
        """
        if hasattr(self.hcm_dataset, 'train_data_indep') and self.hcm_dataset.train_data_indep is not None:
            return self.hcm_dataset.train_data_indep
        else:
            print("Warning: Independent training data not available. Ensure HCMDataset processed correctly.")
            return None

    def get_test_data(self):
        """
        Returns the combined independent testing data (pos + neg) as a NumPy array.
        """
        if hasattr(self.hcm_dataset, 'test_data_indep') and self.hcm_dataset.test_data_indep is not None:
            return self.hcm_dataset.test_data_indep
        else:
            print("Warning: Independent test data not available. Ensure HCMDataset processed correctly.")
            return None

    def get_labels(self):
        """
        Returns labels for the independent test data.
        Labels are the last column of the test_data_indep.
        """
        if hasattr(self.hcm_dataset, 'test_data_indep') and self.hcm_dataset.test_data_indep is not None:
            return self.hcm_dataset.test_data_indep[:, -1]
        else:
            print("Warning: Independent test data not available to extract labels.")
            return np.array([])