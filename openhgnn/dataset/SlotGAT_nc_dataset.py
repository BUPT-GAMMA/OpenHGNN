import os
import torch
import scipy.sparse as sp
import dgl
import numpy as np
# from dgl.data.utils import download, extract_archive
from . import BaseDataset, register_dataset
from collections import Counter, defaultdict
import torch.nn.functional as F

@register_dataset('slotgat')
@register_dataset('acmSlotGAT')
class SlotGATDataset(BaseDataset):
    # _prefix = 'https://s3.cn-north-1.amazonaws.com.cn/dgl-data/'
    # _urls = {
    #     'acmSlotGAT': 'dataset/openhgnn/acmSlotGAT.zip',
    # }
    _local_datasets = {
        'acmSlotGAT': 'ACM',
        'slotgat': 'ACM',
        'ACM': 'ACM',
        'DBLP': 'DBLP',
        'Freebase': 'Freebase',
        'IMDB': 'IMDB',
        'PubMed_NC': 'PubMed_NC',
    }

    def __init__(self, dataset_name, *args, **kwargs):
        super(SlotGATDataset, self).__init__(*args, **kwargs)
        assert dataset_name in self._local_datasets or getattr(kwargs['args'], 'slotgat_data_path', None) is not None
        args = kwargs['args']
        self.args = args
        self.name = dataset_name
        self.raw_dir = getattr(args, 'slotgat_raw_dir', './openhgnn/dataset')
        self.data_root = getattr(args, 'slotgat_data_root', './openhgnn/dataset/NC/data')
        self.path = getattr(args, 'slotgat_data_path', None)
        if self.path is None:
            self.path = os.path.join(self.data_root, self._local_datasets[dataset_name])
        self.data_path = None
        self.extract_dir = self.path
        self.multi_label = getattr(args, 'multi_label', False)
        self.has_feature = True
        self.meta_paths_dict = None
        self.category = getattr(args, 'category', '0')

        source_dataset = kwargs.get('source_dataset', getattr(args, 'slotgat_source_dataset', None))
        if source_dataset is not None:
            self._build_from_standard_dataset(source_dataset, args.device)
        else:
            self.path = self._resolve_data_path(self.path)
            self._build_from_raw_slotgat_files(args.device)
        
    def download(self):
        return None

    def _resolve_data_path(self, explicit_path=None):
        candidates = []
        if explicit_path is not None:
            candidates.append(explicit_path)
        candidates.extend([
            self.extract_dir,
            os.path.join(self.extract_dir, 'data'),
            os.path.join(self.extract_dir, self.name),
            os.path.join(self.extract_dir, self.name, 'data'),
            os.path.join(self.extract_dir, 'ACM'),
            os.path.join(self.extract_dir, 'data', 'ACM'),
        ])
        required_files = ('node.dat', 'link.dat', 'label.dat', 'label.dat.test')
        for candidate in candidates:
            if candidate and all(os.path.exists(os.path.join(candidate, file_name)) for file_name in required_files):
                return candidate
        raise FileNotFoundError(
            f"Cannot find SlotGAT raw files {required_files} under {self.extract_dir}. "
            f"Set args.slotgat_data_path if the archive has a custom layout."
        )

    def _build_from_raw_slotgat_files(self, device):
        self.nodes = self.load_nodes()
        self.links = self.load_links()
        self.labels_train = self.load_labels('label.dat')
        self.labels_test = self.load_labels('label.dat.test')
        self.num_classes = self.labels_train['num_classes']
        self.g = self.load_graph(device)
        self.e_feat = self.get_e_feat()
        self.features_list = self.get_feature_list(device)
        self.in_dim = [feature.shape[1] for feature in self.features_list]
        self.num_ntype = len(self.features_list)
        self.num_etypes = int(self.e_feat.max().item()) + 1 if self.e_feat.numel() > 0 else 0
        if not hasattr(self.args, 'in_dim') or self.args.in_dim is None:
            self.args.in_dim = self.in_dim
        if not hasattr(self.args, 'num_ntype') or self.args.num_ntype is None:
            self.args.num_ntype = self.num_ntype
        if not hasattr(self.args, 'num_etypes') or self.args.num_etypes is None:
            self.args.num_etypes = self.num_etypes
        self.process_g()

    def _build_from_standard_dataset(self, dataset, device):
        g = getattr(dataset, 'g', dataset)
        if not isinstance(g, dgl.DGLHeteroGraph):
            raise TypeError('slotgat_source_dataset must provide a DGLHeteroGraph through `.g` or be a graph itself.')

        self.category = str(getattr(dataset, 'category', self.category))
        self.num_classes = getattr(dataset, 'num_classes', None)
        self.multi_label = getattr(dataset, 'multi_label', self.multi_label)
        self.meta_paths_dict = getattr(dataset, 'meta_paths_dict', None)

        self.features_list, node_slices = self._extract_feature_list_from_heterograph(g, device)
        self.in_dim = [feature.shape[1] for feature in self.features_list]
        self.g, self.e_feat = self._convert_heterograph_to_slotgat_graph(g, node_slices, device)
        self.num_ntype = len(self.features_list)
        self.num_etypes = int(self.e_feat.max().item()) + 1 if self.e_feat.numel() > 0 else 0
        if not hasattr(self.args, 'in_dim') or self.args.in_dim is None:
            self.args.in_dim = self.in_dim
        if not hasattr(self.args, 'num_ntype') or self.args.num_ntype is None:
            self.args.num_ntype = self.num_ntype
        if not hasattr(self.args, 'num_etypes') or self.args.num_etypes is None:
            self.args.num_etypes = self.num_etypes
        self.process_g()

        if hasattr(dataset, 'train_idx'):
            self.train_idx = dataset.train_idx
        if hasattr(dataset, 'valid_idx'):
            self.valid_idx = dataset.valid_idx
        if hasattr(dataset, 'test_idx'):
            self.test_idx = dataset.test_idx
        if hasattr(dataset, 'labels'):
            raw_labels = dataset.labels.to(device) if isinstance(dataset.labels, torch.Tensor) else dataset.labels
            if isinstance(raw_labels, torch.Tensor) and not self.multi_label and raw_labels.dim() > 1:
                raw_labels = raw_labels.argmax(dim=1)
            self.labels = raw_labels

    def load_graph(self, device):
        adj_m = sum(self.links['data'].values())
        g = dgl.from_scipy((adj_m + adj_m.T).tocsr())
        g = dgl.remove_self_loop(g)
        g = dgl.add_self_loop(g)
        return g.to(device)

    def _extract_feature_list_from_heterograph(self, g, device):
        features_list = []
        node_slices = []
        start = 0
        for ntype in g.ntypes:
            num_nodes = g.num_nodes(ntype)
            node_slices.append((start, start + num_nodes))
            start += num_nodes

            node_data = g.nodes[ntype].data
            feature = None
            for key in ('h', 'feat', ntype):
                if key in node_data:
                    feature = node_data[key]
                    break
            if feature is None:
                feature = torch.eye(num_nodes)
            elif not isinstance(feature, torch.Tensor):
                feature = torch.tensor(feature)

            features_list.append(feature.float().to(device) if not feature.is_sparse else feature.to(device))
        return features_list, node_slices

    def _convert_heterograph_to_slotgat_graph(self, g, node_slices, device):
        canonical_etypes = list(g.canonical_etypes)
        homo_g = dgl.to_homogeneous(g)
        homo_g = dgl.remove_self_loop(homo_g)
        edge_types = homo_g.edata[dgl.ETYPE].long()
        homo_g = dgl.add_self_loop(homo_g)

        self.nodes = {
            'count': {i: g.num_nodes(ntype) for i, ntype in enumerate(g.ntypes)},
            'total': homo_g.num_nodes(),
            'shift': {i: node_slices[i][0] for i in range(len(node_slices))},
        }
        self.links = {
            'count': {i: g.num_edges(etype) for i, etype in enumerate(canonical_etypes)},
            'total': int(sum(g.num_edges(etype) for etype in canonical_etypes)),
            'meta': {i: (canonical_etypes[i][0], canonical_etypes[i][2]) for i in range(len(canonical_etypes))},
        }

        if homo_g.num_edges() > len(edge_types):
            extra_self_loops = homo_g.num_edges() - len(edge_types)
            self_loop_types = torch.full(
                (extra_self_loops,), len(canonical_etypes), dtype=torch.long, device=edge_types.device
            )
            edge_types = torch.cat([edge_types, self_loop_types], dim=0)

        etype_ids = defaultdict(list)
        for eid, etype_id in enumerate(edge_types.tolist()):
            etype_ids[etype_id].append(eid)

        homo_g = homo_g.to(device)
        homo_g.etype_ids = dict(etype_ids)
        return homo_g, edge_types.to(device)
        
    def get_feature_list(self, device):
        features = []
        for i in range(len(self.nodes['count'])):
            th = self.nodes['attr'][i]
            if th is None:
                features.append(sp.eye(self.nodes['count'][i]))
            else:
                features.append(th)
        features_list = [self.mat2tensor(feature).to(device) for feature in features]
        save = 0
        for i in range(0, len(features_list)):
            if i == save:
                continue
            features_list[i] = torch.zeros((features_list[i].shape[0], 10)).to(device)
        return features_list
     
    def mat2tensor(self, mat):
        if isinstance(mat, np.ndarray):
            return torch.from_numpy(mat).type(torch.FloatTensor)
        return self.sp_to_spt(mat)

    def sp_to_spt(self, mat):
        coo = mat.tocoo()
        values = coo.data
        indices = np.vstack((coo.row, coo.col))

        i = torch.LongTensor(indices)
        v = torch.FloatTensor(values)
        shape = coo.shape

        return torch.sparse_coo_tensor(i, v, torch.Size(shape), dtype=torch.float32)

    def get_e_feat(self):
        edge2type = {}
        for k in self.links['data']:
            for u, v in zip(*self.links['data'][k].nonzero()):
                edge2type[(u,v)] = k
        count_self = 0
        for i in range(self.nodes['total']):
            FLAG = 0
            if (i, i) not in edge2type:
                edge2type[(i, i)] = len(self.links['count'])
                FLAG = 1
        count_self += FLAG
        count_reverse = 0
        for k in self.links['data']:
            FLAG = 0
            for u, v in zip(*self.links['data'][k].nonzero()):
                if (v, u) not in edge2type:
                    edge2type[(v, u)] = count_reverse + 1 + len(self.links['count'])
                    FLAG = 1
            count_reverse += FLAG
        e_feat = []
        count = 0
        count_mappings = {}
        counted_dict = {}
        eid = 0
        etype_ids = {}
        g_ = self.g.cpu()
        for u, v in zip(*g_.edges()):
            u = u.item() #u.cpu().item()
            v = v.item() #v.cpu().item()
            if not counted_dict.setdefault(edge2type[(u, v)], False):
                count_mappings[edge2type[(u, v)]] = count
                counted_dict[edge2type[(u, v)]] = True
                count += 1
            e_feat.append(count_mappings[edge2type[(u, v)]])
            
            if edge2type[(u, v)] in etype_ids.keys():
                etype_ids[edge2type[(u, v)]].append(eid)
            else:
                etype_ids[edge2type[(u, v)]] = [eid]
            eid += 1
        e_feat = torch.tensor(e_feat, dtype=torch.long).to(self.g.device)
        self.g.etype_ids = etype_ids
        return e_feat
    
    def process_g(self):
        num_ntypes = len(self.features_list)
        num_nodes = self.nodes['total']
        self.g.edge_type_indexer=F.one_hot(self.e_feat).to(self.g.device)
        self.g.node_idx_by_ntype = []
        self.g.num_ntypes = num_ntypes
        self.g.node_ntype_indexer = torch.zeros(num_nodes, num_ntypes).to(self.g.device)
        idx_count = 0
        ntype_count = 0
        for feature in self.features_list:
            temp = []
            for _ in feature:
                temp.append(idx_count)
                self.g.node_ntype_indexer[idx_count][ntype_count] = 1
                idx_count += 1

            self.g.node_idx_by_ntype.append(temp)
            ntype_count += 1 
    
    def get_loss_fn(self):
        return F.nll_loss
        
    def load_links(self):
        """
        return links dict
            total: total number of links
            count: a dict of int, number of links for each type
            meta: a dict of tuple, explaining the link type is from what type of node to what type of node
            data: a dict of sparse matrices, each link type with one matrix. Shapes are all (nodes['total'], nodes['total'])
        """
        links = {'total': 0, 'count': Counter(), 'meta': {}, 'data': defaultdict(list)}
        r_ids = []
        with open(os.path.join(self.path, 'link.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                h_id, t_id, r_id, link_weight = int(th[0]), int(th[1]), int(th[2]), float(th[3])
                if h_id in self.old_to_new_id_mapping.keys() and t_id in self.old_to_new_id_mapping.keys():
                    h_id = self.old_to_new_id_mapping[h_id]
                    t_id = self.old_to_new_id_mapping[t_id]
                    if r_id not in links['meta']:
                        h_type = self.get_node_type(h_id)
                        t_type = self.get_node_type(t_id)
                        links['meta'][r_id] = (h_type, t_type)
                    links['data'][r_id].append((h_id, t_id, link_weight))
                    links['count'][r_id] += 1
                    links['total'] += 1
                    if r_id not in r_ids:
                        r_ids.append(r_id)
        r_ids = sorted(r_ids)
        temp_meta = {}
        for i in range(len(links['meta'].keys())):
            temp_meta[i] = links['meta'][r_ids[i]]
        links['meta'] = temp_meta

        temp_count = {}
        for i in range(len(links['count'].keys())):
            temp_count[i] = links['count'][r_ids[i]]
        links['count'] = temp_count

        temp_data = {}
        for i in range(len(links['data'].keys())):
            temp_data[i] = links['data'][r_ids[i]]
        links['data'] = temp_data

        new_data = {}
        for r_id in links['data']:
            new_data[r_id] = self.list_to_sp_mat(links['data'][r_id])
        links['data'] = new_data
        return links
    
    def get_node_type(self, node_id):
        for i in range(len(self.nodes['shift'])):
            if node_id < self.nodes['shift'][i] + self.nodes['count'][i]:
                return i
    
    def load_labels(self, name):
        """
        return labels dict
            num_classes: total number of labels
            total: total number of labeled data
            count: number of labeled data for each node type
            data: a numpy matrix with shape (self.nodes['total'], self.labels['num_classes'])
            mask: to indicate if that node is labeled, if False, that line of data is masked
        """
        labels = {'num_classes': 0, 'total': 0, 'count': Counter(), 'data': None, 'mask': None}
        nc = 0
        mask = np.zeros(self.nodes['total'], dtype=bool)
        data = [None for i in range(self.nodes['total'])]
        with open(os.path.join(self.path, name), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                node_id, node_name, node_type, node_label = int(th[0]), th[1], int(th[2]), list(map(int, th[3].split(',')))
                for label in node_label:
                    nc = max(nc, label+1)
                mask[node_id] = True
                data[node_id] = node_label
                labels['count'][node_type] += 1
                labels['total'] += 1
        labels['num_classes'] = nc
        new_data = np.zeros((self.nodes['total'], labels['num_classes']), dtype=int)
        for i, x in enumerate(data):
            if x is not None:
                for j in x:
                    new_data[i, j] = 1
        labels['data'] = new_data
        labels['mask'] = mask
        return labels
    
    def list_to_sp_mat(self, li):
        data = [x[2] for x in li]
        i = [x[0] for x in li]
        j = [x[1] for x in li]
        return sp.coo_matrix((data, (i,j)), shape=(self.nodes['total'], self.nodes['total'])).tocsr()
    
    def load_nodes(self):
        """
        return nodes dict
            total: total number of nodes
            count: a dict of int, number of nodes for each type
            attr: a dict of np.array (or None), attribute matrices for each type of nodes
            shift: node_id shift for each type. You can get the id range of a type by 
                        [ shift[node_type], shift[node_type]+count[node_type] )
        """
        nodes = {'total': 0, 'count': Counter(), 'attr': {}, 'shift': {}}
        node_ids = []
        with open(os.path.join(self.path, 'node.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                if len(th) == 4:
                    node_id, node_name, node_type, node_attr = th
                    node_id = int(node_id)
                    node_type = int(node_type)
                    node_attr = list(map(float, node_attr.split(',')))
                    node_ids.append(node_id)
                    nodes['count'][node_type] += 1
                    nodes['attr'][node_id] = node_attr
                    nodes['total'] += 1
                elif len(th) == 3:
                    node_id, node_name, node_type = th
                    node_id = int(node_id)
                    node_type = int(node_type)
                    node_ids.append(node_id)
                    nodes['count'][node_type] += 1
                    nodes['total'] += 1
                else:
                    raise Exception("Too few information to parse!")

        temp_count = Counter()
        for k in nodes['count']:
            temp_count[k] = nodes['count'][k]
        nodes['count'] = temp_count
        self.old_to_new_id_mapping = {}
        self.new_to_old_id_mapping = {}
        node_ids = sorted(node_ids)
        for new_id in range(len(node_ids)):
            self.old_to_new_id_mapping[node_ids[new_id]] = new_id
            self.new_to_old_id_mapping[new_id] = node_ids[new_id]
        temp_attr = {}
        for old_id in node_ids:
            if old_id in nodes['attr'].keys():
                temp_attr[self.old_to_new_id_mapping[old_id]] = nodes['attr'][old_id]
        nodes['attr'] = temp_attr

        shift = 0
        attr = {}
        for i in range(len(nodes['count'])):
            nodes['shift'][i] = shift

            if shift in nodes['attr']:
                mat = []
                for j in range(shift, shift + nodes['count'][i]):
                    mat.append(nodes['attr'][j])
                attr[i] = np.array(mat)
            else:
                attr[i] = None
            shift += nodes['count'][i]
        nodes['attr'] = attr
        return nodes
        
    def get_split(self, validation=True):
        if hasattr(self, 'train_idx') and hasattr(self, 'valid_idx') and hasattr(self, 'test_idx'):
            return self.train_idx, self.valid_idx, self.test_idx

        val_ratio = 0.2
        train_idx = np.nonzero(self.labels_train['mask'])[0]
        np.random.shuffle(train_idx)
        split = int(train_idx.shape[0] * val_ratio)
        val_idx = np.sort(train_idx[:split])
        train_idx = np.sort(train_idx[split:])
        test_idx = np.nonzero(self.labels_test['mask'])[0]

        self.train_idx = torch.LongTensor(train_idx)
        self.valid_idx = torch.LongTensor(val_idx)
        self.test_idx = torch.LongTensor(test_idx)
        return self.train_idx, self.valid_idx, self.test_idx
    
    def get_labels(self):
        if hasattr(self, 'labels'):
            if self.multi_label:
                return self.labels
            if self.labels.dim() > 1:
                return self.labels.argmax(dim=1)
            return self.labels
        if not hasattr(self, 'train_idx') or not hasattr(self, 'valid_idx'):
            self.get_split()

        labels = np.zeros((self.nodes['count'][0], self.labels_train['num_classes']), dtype=int)
        train_idx = self.train_idx.cpu().numpy()
        valid_idx = self.valid_idx.cpu().numpy()
        labels[train_idx] = self.labels_train['data'][train_idx]
        labels[valid_idx] = self.labels_train['data'][valid_idx]
        if not self.multi_label:
            labels = labels.argmax(axis=1)

        if self.multi_label:
            self.labels = torch.FloatTensor(labels).to(self.g.device)
        else:
            self.labels = torch.LongTensor(labels).to(self.g.device)
        return self.labels


    def get_feature(
            self,
    ):
        return self.g.ndata.pop("feat")

    def to(self, device):
        self.g = self.g.to(device)
        self.e_feat = self.e_feat.to(device)
        self.features_list = [feature.to(device) for feature in self.features_list]
        if hasattr(self, 'labels') and isinstance(self.labels, torch.Tensor):
            self.labels = self.labels.to(device)
        return self

class SlotGATDataLoader(SlotGATDataset):
    pass
