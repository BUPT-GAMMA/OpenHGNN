...
class QuaternionConv(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 ...
                 weight_init='quaternion', seed=None, operation='convolution3d', rotation=False, quaternion_format=True, scale=False):
...
class QuaternionLinear(Module):
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(' \
            ...
            + ', init_criterion=' + str(self.init_criterion) \
            + ', weight_init=' + str(self.weight_init) \
            + ', seed=' + str(self.seed) + ')'  + str(self.operation) + ')'


if __name__=="__main__":
    # a=QuaternionConv(4,16,kernel_size=3,stride=2,padding=1)
    # inputs = torch.rand(1, 4, 128, 128, 128)
    # out=a(inputs)‘
    # print(a)
    # m=QuaternionLinear(20,36)
    m = nn.Linear(20, 30)
    input = torch.randn(128, 20)
    output = m(input)
    print(output.size())
    # a=torch.rand(128,120) #只能bs,number
    # b=torch.rand(120,128)
    # out=hamilton_product(a,b) #只能对应乘
    # print(out.size())
    # q1_r = get_r(a)
    # q1_i = get_i(a)
    # q1_j = get_j(a)
