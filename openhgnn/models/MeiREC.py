...
class Model(nn.Module):
    ...
    def _generate_model_layer(self):
        ...
        (
            ...
            self.b_l1,
            self.loss1,
        ) = self._rnn_lstm(5, self.args.batch_num, 64, 64)
    ...
    def regular_loss(self):
        return (
            self.get_regular_loss(self.h_1) + self.get_regular_loss(self.c_1) +
            self.get_regular_loss(self.w_l1) +
            self.get_regular_loss(self.h_2) + self.get_regular_loss(self.c_2) +
            self.get_regular_loss(self.w_l2) +
            self.get_regular_loss(self.h_3) + self.get_regular_loss(self.c_3) +
            self.get_regular_loss(self.w_l3) +
            self.get_regular_loss(self.h_4) + self.get_regular_loss(self.c_4) +
            self.get_regular_loss(self.w_l4) +
            self.get_regular_loss(self.conv_w1) +
            self.get_regular_loss(self.conv_w2) +
    ...
    def get_weights_variables(self, shape, weight_decay, trainable=True):
        ...
        if weight_decay == 0:
            ...
            nn.init.xavier_normal_(params, gain=1.0)
    ...
    def forward(self, x):
        ...
        inputs_query_raw = self._word_embed[query_terms.reshape(
            -1).long()].view(self.config.query_length, -1,
                             self.config.embed_size_word)
        ...
        self.query_w2v_sum = torch.mean(
            inputs_query_raw[:int(input_num.item())], 0).T
        ...
        for raw_word_embed in raw_word_embedding_list:
            item_terms, item_topcate, item_leafcate, time_delta = torch.split(
                ...
                0)
            step_embedding = self._word_embed[item_terms.reshape(
                -1).long()].view(self.config.user_item_term_length, -1,
                                 self.config.embed_size_word)
            input_num = (torch.sum(torch.sign(query_terms))
                         if torch.sum(torch.sign(query_terms)) > 1 else
                         torch.tensor(1))
        ...
        for raw_user_item_embed in raw_user_item_query_embedding_list:
            ...
            step_embedding = self._word_embed[item_terms.reshape(
                -1).long()].view(5, -1, self.config.embed_size_word)
            input_num = (torch.sum(torch.sign(query_terms))
                         if torch.sum(torch.sign(query_terms)) > 1 else
                         torch.tensor(1))
            step_avg_embedding = torch.mean(
        ...
        for raw_user_item_embed in raw_user_query_seq_embedding_list[::-1]:
            ...
            step_embedding = self._word_embed[query_terms.reshape(
                -1).long()].view(self.config.user_query_term_length, -1,
                                 self.config.embed_size_word)
            input_num = (torch.sum(torch.sign(query_terms))
                         if torch.sum(torch.sign(query_terms)) > 1 else
                         torch.tensor(1))
        ...
        convd = self.query_item_query_cnn(
            step_embedding_vec.view(-1, 1, 5, self.config.embed_size_word))
        ...
        self.wide_hidden_layer1 = torch.tanh(
            F.dropout(
