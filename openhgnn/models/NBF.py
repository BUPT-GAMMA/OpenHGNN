...
class NBFNet(BaseModel):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
        ...
        self.layers = nn.ModuleList()#这个layers是最重要的层
    ...
    def bellmanford(self, data, h_index, r_index, separate_grad=False):
        ...
        # initialize queries (relation types of the given triples) ,query是所有三元组的关系类型
        query = self.query(r_index)
        index = h_index.unsqueeze(-1).expand_as(query)

        # initial (boundary) condition - initialize all node states as zeros ，boundary是初始的所有节点的节点状态
        ...
        hiddens = []
        edge_weights = []
        layer_input = boundary#boundary就是初始的node_feat

        for layer in self.layers:#用到layer.forward
            if separate_grad:
    ...
    def forward(self, data, batch):#    data == train_data
        ...
        if self.training:
            ...
            data = self.remove_easy_edges(data, h_index, t_index, r_index) #    边数量 10820 --> 10692

        # 新加内容
        data.num_edges = data.edge_index.shape[1] # 新加内容
...
################  下一行是自己加的代码，原本在Messagepassing类中
        self.node_dim = -2
####################################
...
# NBFNet model中的调用：hidden = layer(layer_input, query, boundary, data.edge_index, data.edge_type, size, edge_weight),size = (data.num_nodes, data.num_nodes)

#下面这个函数是原来的forward，不要删
    # def __forward(self, input, query, boundary, edge_index, edge_type, size, edge_weight=None):
    #     batch_size = len(query)
    #     #input是当前层每个节点的state，query是每个三元组的关系类型
    #     # input.shape == 64,2746,32  ,query.shape == 64,32  ,edge_index,edge_type,edge_weight都是10692条边
    ...
    #     # forward调用propagate(),而propagate()依次调用message,aggregate,update
    ...
    #     ###  以下这个是修改后的代码,size = (data.num_nodes, data.num_nodes)
    #     # output = self.__propagate(input=input, relation=relation, boundary=boundary, edge_index=edge_index,
    #     #                         edge_type=edge_type, size=size, edge_weight=edge_weight)
    ...
    def message(self, input_j, relation, boundary, edge_type):#j是源节点
        ...
        # message shape == 64,13438,32
        return message

    def aggregate(self, input, edge_weight, index, dim_size):
        # input shape == 64,13438,32==message的返回值,  edge_weight.shape == 10692 ,index shape == 10692  ,dim_size ==2746(num_nodes)
        # 我推测，index代表的是每条边的target节点的编号，dim_size是节点个数
        ...
        if self.aggregate_func == "pna":
            ...
            degree_out = degree(index, dim_size).unsqueeze(0).unsqueeze(-1)#PYG
        ...
        #output shape == 64,2476,384
        return output

    def update(self, update, input):
        # update.shape == 64,2476,384 ，即aggregate的返回值
        # input.shape == 64,2746,32 ，这里input应该是原始的全部节点的state，不分源和目的
    ...
    def message_and_aggregate(self, edge_index, input, relation, boundary, edge_type, edge_weight, index, dim_size):
        ...
        # input shape == [64, 2746, 32]  == [btsz,num_nodes,input_dim]，t,v,t节点数2746,2746,922
    ...
    def propagate(self, edge_index, size=None, **kwargs):
        ...
        for hook in self._propagate_forward_pre_hooks.values():#MESSAGEPASSING
            res = hook(self, (edge_index, size, kwargs))
            if res is not None:
                edge_index, size, kwargs = res

        size = self._check_input(edge_index, size)
        coll_dict = self._collect(self._fused_user_args, edge_index,#父类的读取参数
                                     size, kwargs)

        msg_aggr_kwargs = self.inspector.distribute("message_and_aggregate", coll_dict)#读取参数
        for hook in self._message_and_aggregate_forward_pre_hooks.values():#父类的成员
            res = hook(self, (edge_index, msg_aggr_kwargs))
            if res is not None:
                edge_index, msg_aggr_kwargs = res
        out = self.message_and_aggregate(edge_index, **msg_aggr_kwargs)
        for hook in self._message_and_aggregate_forward_hooks.values():#父类的成员
            res = hook(self, (edge_index, msg_aggr_kwargs), out)
            if res is not None:
                out = res

        update_kwargs = self.inspector.distribute("update", coll_dict)#父类的成员
        out = self.update(out, **update_kwargs)

        for hook in self._propagate_forward_hooks.values():#父类的成员
...
#这个是【不用c++加速的+原版PYG的】函数，forward中会依次调用message,aggregate,update
    # def __propagate(self, edge_index, size=None, **kwargs):
