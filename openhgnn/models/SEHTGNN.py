...
class LLM4init(nn.Module):
    def __init__(self, graph, n_hid, LLM_feature, device):
        ...
        if LLM_feature is not None and len(LLM_feature) > 0:
            llm_dim = list(LLM_feature.values())[0].shape[1]
        else:
            llm_dim = 4096
        self.consistent = nn.Linear(llm_dim, n_hid).to(device) 
    ...
    def forward(self):
        ...
        for k, v in feature_dict.items():
            projected_feat[k] = self.consistent(v) 

        grouped_edges = defaultdict(list)
        for stype, reltype, dtype in self.reltype:
            stype_feat = projected_feat[stype]
            dtype_feat = projected_feat[dtype]
    ...
    def __init__(self, graph, n_inp, n_hid, timeframe, norm):
        ...
        for srctype, etype, dsttype in graph.canonical_etypes:
            ...
            if real_rel_name not in self.predict:
                self.predict[real_rel_name] = Dyanmicatt(n_inp, n_hid)
        if norm:
    ...
    def forward(self, graph, node_features, init_attention):
        ...
        for stype, etype, dtype in graph.canonical_etypes:
            ...
            if ttype in node_features[stype] and ttype in node_features[dtype]:
                dst_feat = self.intra_rel_agg(graph[stype, etype, dtype], 
        ...
        for reltype, feats in h_mask_collector.items():
            ...
            if reltype in init_attention:
                h0 = init_attention[reltype].expand(1, stacked_feats.size(0), 1).contiguous()
        ...
        for ttype in self.timeframe:
            for ntype in graph.ntypes:
                ...
                if not collector:
                    if ttype in node_features[ntype]: inter_features[ntype][ttype] = node_features[ntype][ttype]
                    continue
                w_norm = F.softmax(torch.stack(weights, dim=0), dim=0)
                out_feat = sum([collector[i] * w_norm[i] for i in range(len(collector))])
                if self.norm: inter_features[ntype][ttype] = self.norm_layer[ntype](out_feat)
                else: inter_features[ntype][ttype] = out_feat + node_features[ntype][ttype]
...
class SEHTGNN(BaseModel):
    ...
    def build_model_from_args(cls, args, hg):
        ...
        if llm_feat is not None and isinstance(llm_feat, dict):
            print(f"[SEHTGNN] LLM feature dimension: {list(llm_feat.values())[0].shape[1]}")
        n_inp = 1
        print(f"[SEHTGNN] Model raw input dimension: {n_inp}")
    ...
    def __init__(self, graph, n_inp, n_hid, n_layers, time_window, norm, device, dropout, LLM_feature):
        ...
        if LLM_feature is not None: self.LLM_init = LLM4init(graph, n_hid, LLM_feature, device)
        else:
             print("Initializing with Random Semantics...")
             dummy_feat = {nt: torch.randn(1, n_hid).to(device) for nt in graph.ntypes}
             self.LLM_init = LLM4init(graph, n_hid, dummy_feat, device)
        self.gnn_layers = nn.ModuleList([HTGNNLayer(graph, n_hid, n_hid, self.timeframe, norm) for _ in range(n_layers)])
    ...
    def forward(self, hg, h_dict=None):
        ...
        for ntype in hg.ntypes:
            for ttype in self.timeframe:
                if ttype in hg.nodes[ntype].data:
                    ...
                    if raw_feat.dtype != torch.float32: raw_feat = raw_feat.float()
        ...
        for ntype in hg.ntypes:
            ...
            for tt in self.timeframe:
                if tt in spatial_feat[ntype]: feats_to_stack.append(spatial_feat[ntype][tt])
            if len(feats_to_stack) == len(self.timeframe):
                # 修复变量名 feats -> feats_to_stack
