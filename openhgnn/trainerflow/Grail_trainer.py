...
class GrailTrainer(BaseFlow):
    def __init__(self,args):
        super(GrailTrainer, self).__init__(args)
        ...
        self.args.collate_fn = collate_dgl
    ...
    def process_files(self,files, saved_relation2id, add_traspose_rels):
        '''
        ...
        dgl_adj_list = self.ssp_multigraph_to_dgl(adj_list_aug)
    ...
    def get_neg_samples_replacing_head_tail_all(self,test_links, adj_list):
    ...
    def get_neg_samples_replacing_head_tail_from_ruleN(self,ruleN_pred_path, entity2id, saved_relation2id):
        ...
        for i in range(len(pred_data) // 3):
            ...
            if pred_data[3 * i][1] in saved_relation2id:
                head, rel, tail = entity2id[pred_data[3 * i][0]], saved_relation2id[pred_data[3 * i][1]], entity2id[
                    pred_data[3 * i][2]]
    ...
    def incidence_matrix(self,adj_list):
        '''
    ...
    def _bfs_relational(self,adj, roots, max_nodes_per_hop=None):
        """
        ...
        while current_lvl:
            ...
            next_lvl = self._get_neighbors(adj, current_lvl)
    ...
    def _get_neighbors(self,adj, nodes):
        """Takes a set of nodes and a graph adjacency matrix and returns a set of neighbors.
        Directly copied from dgl.contrib.data.knowledge_graph"""
        sp_nodes = self._sp_row_vec_from_idx_list(list(nodes), adj.shape[1])
        sp_neighbors = sp_nodes.dot(adj)
        neighbors = set(ssp.find(sp_neighbors)[1])  # convert to set of indices
        return neighbors

    def _sp_row_vec_from_idx_list(self,idx_list, dim):
    ...
    def get_neighbor_nodes(self,roots, adj, h=1, max_nodes_per_hop=None):
        bfs_generator = self._bfs_relational(adj, roots, max_nodes_per_hop)
        lvls = list()
        for _ in range(h):
            try:
    ...
    def subgraph_extraction_labeling(self,ind, rel, A_list, h=1, enclosing_sub_graph=False, max_nodes_per_hop=None,
        ...
        A_incidence = self.incidence_matrix(A_list)
        ...
        labels, enclosing_subgraph_nodes = self.node_label_new(self.incidence_matrix(subgraph), max_distance=h)
    ...
    def remove_nodes(self,A_incidence, nodes):
        idxs_wo_nodes = list(set(range(A_incidence.shape[1])) - set(nodes))
        return A_incidence[idxs_wo_nodes, :][:, idxs_wo_nodes]

    def node_label_new(self,subgraph, max_distance=1):
        # an implementation of the proposed double-radius node labeling (DRNd   L)
        roots = [0, 1]
        sgs_single_root = [self.remove_nodes(subgraph, [root]) for root in roots]
        dist_to_roots = [
    ...
    def ssp_multigraph_to_dgl(self,graph, n_feats=None):
        """
    ...
    def get_subgraphs(self, all_links, adj_list, dgl_adj_list, max_node_label_value, id2entity, node_features=None,
        ...
        for link in all_links:
            ...
            nodes, node_labels = self.subgraph_extraction_labeling((head, tail), rel, adj_list, h=self.args.hop,
                                                              enclosing_sub_graph=self.args.enclosing_sub_graph,
            ...
            subgraph = self.prepare_features(subgraph, node_labels, max_node_label_value, n_feats)
    ...
    def save_to_file(self,neg_triplets, id2entity, id2relation):

        with open(os.path.join(
                self.params.save_path,'ranking_head.txt'),
                  "w") as f:
        ...
        with open(os.path.join(
                self.params.save_path,'ranking_tail.txt'),
                  "w") as f:
            for neg_triplet in neg_triplets:
                for s, o, r in neg_triplet['tail'][0]:
                    f.write('\t'.join([id2entity[s], id2relation[r], id2entity[o]]) + '\n')

    def save_score_to_file(self,neg_triplets, all_head_scores, all_tail_scores, id2entity, id2relation):

        with open(os.path.join(
                self.params.save_path,'grail_ranking_head_predictions.txt'),
                  "w") as f:
        ...
        with open(os.path.join(
                self.params.save_path,'grail_ranking_tail_predictions.txt'),
                  "w") as f:
            for i, neg_triplet in enumerate(neg_triplets):
    ...
    def save_score_to_file_from_ruleN(self, neg_triplets, all_head_scores, all_tail_scores, id2entity, id2relation):

        with open(os.path.join(
                self.params.save_path,'grail_ruleN_ranking_head_predictions.txt'),
                  "w") as f:
            for i, neg_triplet in enumerate(neg_triplets):
                for [s, o, r], head_score in zip(neg_triplet['head'][0], all_head_scores[50 * i:50 * (i + 1)]):
                    f.write('\t'.join([id2entity[s], id2relation[r], id2entity[o], str(head_score)]) + '\n')
        ...
        with open(os.path.join(
                self.params.save_path,'grail_ruleN_ranking_tail_predictions.txt'),
                  "w") as f:
            for i, neg_triplet in enumerate(neg_triplets):
                for [s, o, r], tail_score in zip(neg_triplet['tail'][0], all_tail_scores[50 * i:50 * (i + 1)]):
                    f.write('\t'.join([id2entity[s], id2relation[r], id2entity[o], str(tail_score)]) + '\n')
...
class Evaluator():
    ...
    def eval(self, save=False):
        ...
        if save:
            pos_test_triplets_path = os.path.join(self.params.save_path, 'data/{}/{}.txt'.format(self.params.dataset, self.data.file_name))
            with open(pos_test_triplets_path) as f:
                pos_triplets = [line.split() for line in f.read().split('\n')[:-1]]
            pos_file_path = os.path.join(self.params.save_path, 'data/{}/grail_{}_predictions.txt'.format(self.params.dataset, self.data.file_name))
            with open(pos_file_path, "w") as f:
                for ([s, r, o], score) in zip(pos_triplets, pos_scores):
                    f.write('\t'.join([s, r, o, str(score)]) + '\n')

            neg_test_triplets_path = os.path.join(self.params.save_path, 'data/{}/neg_{}_0.txt'.format(self.params.dataset, self.data.file_name))
            with open(neg_test_triplets_path) as f:
                neg_triplets = [line.split() for line in f.read().split('\n')[:-1]]
            neg_file_path = os.path.join(self.params.save_path, 'data/{}/grail_neg_{}_{}_predictions.txt'.format(self.params.dataset, self.data.file_name, self.params.constrained_neg_prob))
