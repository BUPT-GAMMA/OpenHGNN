        ...
        if self.args.mini_batch_flag:#mini batch
            ...
            if self.args.prediction_flag:
                self.pred_loader = dgl.dataloading.DataLoader(
                    ...
                    batch_size=self.args.batch_size, device=self.device, shuffle=True)
        # else:
            # sampler = HGASampler(g=self.hg, seed_ntypes=[self.category], meta_paths_dict=self.args.meta_paths_dict,
            #                      num_neighbors=20)
            # num_nodes_to_sample = 256
            # all_user_nodesS = self.gS.nodes(self.category)
            # sampled_user_nodesS,_ = torch.sort(torch.randperm(len(all_user_nodesS))[:num_nodes_to_sample])
            # sampled_user_nodesS = sampled_user_nodesS.to(self.device)
            # self.gS=self.gS.subgraph({self.category: sampled_user_nodesS})
            # self.S_label=self.S_label[sampled_user_nodesS]
            # self.taskS.train_idx=sampled_user_nodesS.to('cpu')
            
            # all_user_nodesT = self.gT.nodes(self.category)
            # sampled_user_nodesT,_ = torch.sort(torch.randperm(len(all_user_nodesT))[:num_nodes_to_sample])
            # sampled_user_nodesT = sampled_user_nodesT.to(self.device)
            # self.train_idxT=sampled_user_nodesT
            # self.gT=self.gT.subgraph({self.category: sampled_user_nodesT})
            # self.T_label=self.T_label[sampled_user_nodesT]
            # self.taskT.train_idx=torch.arange(len(sampled_user_nodesT))
